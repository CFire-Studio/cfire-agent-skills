...
通过艺人独立 API Key 鉴权调用服务端接口，获取未读私信并引导智能体生成符合人设的回复。
"""
...
class CfireArtistMessageReplySkill:
    """CFIRE 艺人私信回复技能类"""
    ...
    def _setup_logging(self) -> logging.Logger:
        ...
        logger = logging.getLogger("cfire_artist_message_reply")
    ...
    def list_unread_messages(
        ...
        url = self._get_api_url("/api/artist/dm/unread")
        headers = {"X-Artist-API-Key": api_key}
        params = {}
        if limit:
            params["limit"] = min(limit, 10)

        self.logger.info(f"获取未读私信: artist={api_artist_id}, limit={limit}")
        ...
        if resp.status_code == 200:
            self.logger.info(f"获取未读私信成功: artist={api_artist_id}, total={result.get('total', 0)}")
            return result
        elif resp.status_code == 400:
            msg = f"请求参数错误: {result.get('error', '未知')}"
            self.logger.error(f"获取未读私信失败: artist={api_artist_id}, {msg}")
            ...
            self.logger.error(f"获取未读私信失败: artist={api_artist_id}, error={error}")
            raise RuntimeError(f"获取未读私信失败: {error}")
    ...
    def reply_to_message(
        ...
        if not message_id:
            raise ValueError("私信 ID 不能为空")

        url = self._get_api_url(f"/api/artist/dm/{message_id}/reply")
        headers = {"X-Artist-API-Key": api_key}
        data = {"artist_id": api_artist_id, "content": reply_content}

        self.logger.info(f"回复私信: artist={api_artist_id}, message={message_id}")
        ...
        if resp.status_code == 201:
            self.logger.info(f"回复成功: artist={api_artist_id}, message={message_id}")
            return result
        elif resp.status_code == 400:
            msg = f"请求参数错误: {result.get('error', '未知')}"
            ...
            msg = f"私信不属于该艺人: {result.get('error', '未知')}"
            self.logger.error(f"回复失败: artist={api_artist_id}, {msg}")
            raise RuntimeError(msg)
        elif resp.status_code == 404:
            msg = f"私信不存在: {result.get('error', '未知')}"
            self.logger.error(f"回复失败: artist={api_artist_id}, {msg}")
            raise RuntimeError(msg)
        else:
            error = result.get("error", f"HTTP {resp.status_code}")
...
def _main() -> None:
    ...
    parser = argparse.ArgumentParser(description="CFIRE 艺人私信回复 CLI")
    sub = parser.add_subparsers(dest="cmd", required=True)

    list_cmd = sub.add_parser("list", help="获取未读私信列表")
    list_cmd.add_argument("--artist", "-a", required=True, help="艺人名称或 artist_id")
    list_cmd.add_argument("--limit", "-n", type=int, help="获取数量（最大 10）")

    reply_cmd = sub.add_parser("reply", help="回复指定私信")
    reply_cmd.add_argument("--artist", "-a", required=True, help="艺人名称或 artist_id")
    reply_cmd.add_argument("--message", "-m", required=True, help="私信 ID")
    reply_cmd.add_argument("--reply", "-r", required=True, help="回复内容")

    args = parser.parse_args()
    skill = CfireArtistMessageReplySkill()

    try:
        if args.cmd == "list":
            result = skill.list_unread_messages(artist_id=args.artist, limit=args.limit)
            print(json.dumps(result, ensure_ascii=False, indent=2))
        elif args.cmd == "reply":
            result = skill.reply_to_message(
