...
- 生成选题策划：选题名称、内容支柱、主题描述、与过往内容的呼应、与阶段目标的关联、预期情绪价值
...
- 选题必须呼应当前阶段目标中的至少一项具体行动，明确说明内容与目标的关联
...
- 在日记中融入对阶段目标的思考——今天为目标做了什么推进，或明天可以做些什么
...
- 日记必须包含「今日目标行动」章节，明确关联当前阶段目标中的具体任务
...
| 存储 | 路径 / 表 | 说明 |
| --- | --- | --- |
| 独立日记文件 | `assets/diary/YYYY-MM-DD.md` | 按日期命名，独立 Schema 校验；作为内容生成的权威引用源 |
...
### 保存时校验项
1. 日期格式与范围
2. `## 正文` / `## 情绪` / `## 今日目标行动` 必需章节存在
3. 情绪标签属于白名单
4. 正文长度 1 ~ 200 字
5. 正文是第一人称，语言风格符合人设
6. 「今日目标行动」需明确关联当前阶段目标中的具体任务
...
3. 文件名日期与 `## 日期` 内容一致
4. 内容支柱属于白名单（如填写）
5. 选题策划必须包含「与阶段目标的关联」章节且内容不为空
6. 拍摄脚本至少包含 2 个镜头
7. 每个镜头必须包含：景别、光线、音效、画面字段
8. 如涉及 Cosplay，「角色妆造」章节建议包含：Cosplay 角色、选择理由、服装、妆容
...
python scripts/skill.py save -d 2026-06-18 -m "期待" \
  -c "今天把昨天在屋顶录的风声切片放进新 demo 里，突然有了副歌的走向。"
...
{
  "date": "2026-06-18",
  "content": "今天把屋顶录的风声切片放进新 demo，副歌忽然有了方向。",
  "mood": "期待",
  "length": 32,
  "source": "llm",
  "saved": false
...
- 未找到 `cfire-artist-profile` 目录时，提示使用 `--profile-dir` 指定。
- `--date` 不合法时直接报错。
